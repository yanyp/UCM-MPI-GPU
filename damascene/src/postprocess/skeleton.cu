...
void skeletonize(int width, int height, int matrixPitchInFloats, float* devGPb_thin)
{
    ...
    customCheckCudaErrors(cudaMemcpyToSymbol(devLut, lutc, NUM_LUTS*512*sizeof(int)));
    
    //bind image to texture
      
    cudaArray* imageArray;
    cudaChannelFormatDesc floatTex = cudaCreateChannelDesc<float>();
    customCheckCudaErrors(cudaMallocArray(&imageArray, &floatTex, matrixPitchInFloats , height));
    customCheckCudaErrors(cudaMemcpyToArray(imageArray, 0, 0, devGPb_thin, matrixPitchInFloats * height * sizeof(float), cudaMemcpyDeviceToDevice));
    ...
    int* devchange;
    customCheckCudaErrors(cudaMalloc((void**)&devchange, sizeof(int)));
    ...
    log_info("Skeletonizing ...");
    do 
    {
        customCheckCudaErrors(cudaMemset(devchange, 0, sizeof(int)));
        for(int lutid = 0; lutid < NUM_LUTS; lutid++)
        {
            applyLut<<<grid, block>>> (width, height, matrixPitchInFloats, devGPb_thin, lutid, devchange);
            customCheckCudaErrors(cudaMemcpyToArray(imageArray, 0, 0, devGPb_thin, matrixPitchInFloats * height * sizeof(float), cudaMemcpyDeviceToDevice));
        }
        customCheckCudaErrors(cudaMemcpy(&change, devchange, sizeof(int), cudaMemcpyDeviceToHost));
        iter++;

        //printf("\tIteration = %d, %s\n", iter, (change > 0) ? "Image changed":"Image unchanged");
        log_info("\tIteration = %d, %s", iter, (change > 0) ? "Image changed":"Image unchanged");
    }while(change != 0);

    customCheckCudaErrors(cudaFree(devchange));
