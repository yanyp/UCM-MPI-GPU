...
void PrintArray(int size, float* devArray)
{
	...
	customCheckCudaErrors( cudaMemcpy(resu, devArray, xdim*ydim, cudaMemcpyDeviceToDevice) );
...
//            } 
	...
	customCheckCudaErrors( cudaMallocArray(&a_Data, &floatTex, xdim_mirrored, ydim_mirrored) );
	customCheckCudaErrors( cudaBindTextureToArray(texData, a_Data) );		
	
	//ts1 = timestamp();	
	
	// allocate space for result
	customCheckCudaErrors( cudaMalloc((void **)&d_Result, imagesize_mirrored*NUM_ORIENT) );
	customCheckCudaErrors( cudaMemset( d_Result, 0, imagesize_mirrored*NUM_ORIENT) );
	//h_Result = (float *) malloc(imagesize_mirrored*NUM_ORIENT);	
	
	// copy kernels to constant memory space
	customCheckCudaErrors( cudaMemcpyToSymbol(d_Kernels, h_Kernels, KERNEL_SIZE*NUM_ORIENT*sizeof(float)) );	
  ...
  dim3 blockGrid(iDivUp(xdim_mirrored, threadBlock.x), iDivUp(ydim_mirrored, threadBlock.y));
   ...
   for (int i=1;i<nvec;i++) {
		...
		customCheckCudaErrors( cudaMemcpyToArray(a_Data, 0, 0, eigenvector, imagesize_mirrored, cudaMemcpyDeviceToDevice) );
				
        customCheckCudaErrors( cudaThreadSynchronize() );
...
/*
	customCheckCudaErrors( cudaMemcpy(h_Result, d_Result, imagesize_mirrored*NUM_ORIENT, cudaMemcpyDeviceToHost) );
   for(int i=0;i<NUM_ORIENT;i++) {
	   int y;
	   float *result;
...
*/
   ...
   customCheckCudaErrors( cudaUnbindTexture(texData) );
   customCheckCudaErrors( cudaFree(d_Result)   );
   customCheckCudaErrors( cudaFreeArray(a_Data)   );
   free(h_Kernels);
}  
