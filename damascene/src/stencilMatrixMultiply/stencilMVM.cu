...
int findPitchInFloats(int width) {
  float* test;
  size_t pitch;
  customCheckCudaErrors(cudaMallocPitch((void**)&test, &pitch, width * sizeof(float), 1));
  cudaFree(test);
...
/* /\*   printf("\n"); *\/ */
  ...
  float* devSum;
  customCheckCudaErrors(cudaMalloc((void**)&devSum, width * sizeof(float) * height));
  float* devRSqrtSum;
  customCheckCudaErrors(cudaMalloc((void**)&devRSqrtSum, width * sizeof(float) * height));
...
/* /\*   customCheckCudaErrors(cudaMemcpy(hostSum, devSum, width * sizeof(float) * height, cudaMemcpyDeviceToHost)); *\/ */
...
/* insert stuff here  
  ...
  customCheckCudaErrors(cudaMemcpy(hostMatrix, devMatrix, matrixPitchInFloats * sizeof(float) * nDimension, cudaMemcpyDeviceToHost)); 
...
/*   memset(hostSum, 0, width * height * sizeof(float)); */
  /* float* devSum; */
/*   size_t devSumPitch; */
/*   customCheckCudaErrors(cudaMallocPitch((void**)&devSum, &devSumPitch, width *sizeof(float), height)); */
/*   stencilSumRows<<<gridDim, blockDim>>>(width, height, nPixels, radius, nDimension, devMatrix, widthPitchInFloats, devSum); */

 /*  customCheckCudaErrors(cudaMemcpy2D(hostSum, width * sizeof(float), devSum, devSumPitch, width*sizeof(float), height, cudaMemcpyDeviceToHost)); */
/*   for(int row = 0; row < height; row++) { */
/*     for (int col = 0; col < width; col++) { */
/*       printf("%f ", hostSum[row * width + col]); */
/*     } */
