...
void PrintVectorOnFile(int p_nSize, float* vec, char* filename)
{
	...
	customCheckCudaErrors(cudaMemcpy(tempVec, vec, p_nSize*sizeof(float), cudaMemcpyDeviceToHost));
...
void initEigs(int p_nEigNum, int p_nMatrixDimension, float** p_eigenValues, float** devEigVectors)
{
  ...
  customCheckCudaErrors(cudaMalloc((void**)devEigVectors, p_nMatrixDimension * sizeof(float)*p_nEigNum));
                 ...
                 float** p_aTEigVals, float** p_aaTEigVecs)
{
	...
	if (status != CUBLAS_STATUS_SUCCESS)
	{
		...
		log_error("!!!! CUBLAS initialization error");
		return;
	}
						...
						float* p_daaDMatrixV, float* p_daVectorS, float* p_daVectorX, float p_fTolerance)
{
    ...
    float absoluteResidual = abs(p_aBeta[p_nIter]*p_aaTEigVecs[i * (p_nIter + 1) + p_nIter]);
      ...
      log_warn("Eigenvalue: %d has too large a residual %e", i, absoluteResidual);
			return false;
    }
  }
 /*  for (int i = p_nEigNum - 1; i >= 0; i--) { */
              ...
              float* p_daaDMatrixV, float* devRSqrtSum)
{
	...
	float* devTEigVecs;
	customCheckCudaErrors(cudaMallocPitch((void**)&devTEigVecs, &tEigenVectorPitch, tVecLength * sizeof(float), p_nEigNum));
...
///
	//printf("Iteration %d : Read from %d -- Write to %d storing = %d\n", iteration, read, write, storeVectors );
        customCheckCudaErrors(cudaMemcpy(devVector, d_aVectorQQ, nPixels * sizeof(float), cudaMemcpyDeviceToDevice));
	...
	if(i==maxIterationsThatFitGPU-1 || iteration>=nIterations-1)
	{
		//cudamemcpy to CPU --all the lanczos vectors
                if(storeVectors)
                {
                    ...
                    float *RitzGPU=0;
                    size_t RitzGPUPitch;
                    customCheckCudaErrors(cudaMallocPitch((void**)&RitzGPU, &RitzGPUPitch, sizeof(float)*IterationsToDo, p_nEigNum));
                    customCheckCudaErrors(cudaMemcpy2D(RitzGPU, RitzGPUPitch, RitzVectors+k*(maxIterationsThatFitGPU-1), (nIterations+1)*sizeof(float), IterationsToDo*sizeof(float), p_nEigNum, cudaMemcpyHostToDevice));
                    assert(RitzGPU != NULL); 
                    ...
                    customCheckCudaErrors(cudaFree(RitzGPU));
		...
		customCheckCudaErrors(cudaMemcpy( d_aaDMatrixV ,  d_aaDMatrixV + (i)*p_nMatrixDimension, sizeof(float)*p_nMatrixDimension*2, cudaMemcpyDeviceToDevice));
             ...
             int p_nEigNum, float* p_eigenValues, float* devEigVectors, int p_nOrthoChoice, float* devRSqrtSum, float p_fTolerance)
{
	...
	if(ce != cudaSuccess)
	{
		...
		log_error("Error in line %d in %s : %s",__LINE__,__FILE__, cudaGetErrorString(ce));
		//return;
	}
        ...
        do {
            ...
            log_info("Can fit %d iterations on GPU", maxIterationsThatFitGPU);
            ...
            if(ce != cudaSuccess)
            {
                    ...
                    log_error("Cuda alloc failed -- trying to make do with less memory");
        ...
        if(margin <= 0)
        {
            ...
            log_fatal("Aborted due to insufficient memory");
            exit(-1);
        }
  ...
  bindTexture(devVector, nPixels);
	...
	for (int iter = 0; iter < MAXITER; iter++)
	{
		...
		if ((i % 100) == 0) {
			...
			log_info("lanczos iteration: %d", i);
		}
		...
		if (((i+1) % CHECKITER == 0) || (i == MAXITER - 1) || (i == maxIterationsThatFitGPU-2))
		{
			...
			while (1)
			{
				...
				log_warn("Screened Eig number too small, enlarge as %d", eigCheck);
		...
		times.push_back(currentIteration);
                ...
                if(ce != cudaSuccess)
                {
                    ...
                    log_error("Error %d in %s : %s",__LINE__,__FILE__, cudaGetErrorString(ce));
                    //return;
                }
        ...
        log_info("nIterations = %d", i+1);

        size_t eigenVectorPitch;
        customCheckCudaErrors(cudaMemset(devEigVectors, 0, p_nMatrixDimension * sizeof(float)*p_nEigNum));
        //customCheckCudaErrors(cudaMallocPitch((void**)&devEigVectors, &eigenVectorPitch, p_nMatrixDimension * sizeof(float), p_nEigNum));
        //customCheckCudaErrors(cudaMemset(devEigVectors, 0, eigenVectorPitch*p_nEigNum));
	...
	gettimeofday(&lanczosTimeStop, 0);
        ...
        log_info("lanczos Iterations : %f seconds", getTimeUs(lanczosTimeStart, lanczosTimeStop)/1e6);
	...
	log_info("Eigenvector calculation: %f microseconds", getTimeUs(eigCalcStart, eigCalcStop));
...
void PrintCudaVector(int p_nSize, float* p_devVec)
{
	...
	customCheckCudaErrors(cudaMemcpy(vec, p_devVec, p_nSize*sizeof(float), cudaMemcpyDeviceToHost));
	for (int i = 0; i < p_nSize; i++)
	{
		//printf("\n %d : %f", i, vec[i]);
		log_info("%d : %f", i, vec[i]);
	}
	free(vec);
}
...
void FindMaxMin(int p_nStart, int p_nEnd, float* p_Vec)
{
	...
	log_info("Serial Max %f Min %f", max, min);
}
...
void NormalizeEigVecDev(int p_nMatrixDimension, float* p_devEig, int p_nEigNum)
{
	...
	customCheckCudaErrors(cudaMalloc((void**)&devReduceMin, blockNum*sizeof(float)*(MAXEIGNUM-1)));
	customCheckCudaErrors(cudaMalloc((void**)&devReduceMax, blockNum*sizeof(float)*(MAXEIGNUM-1)));
	...
	customCheckCudaErrors(cudaMalloc((void**)&devFinalMax, MAXEIGNUM*sizeof(float)));
	customCheckCudaErrors(cudaMalloc((void**)&devFinalMin, MAXEIGNUM*sizeof(float)));
	dim3 oneGrid(1,1);
	...
	customCheckCudaErrors(cudaFree(devReduceMax));
	customCheckCudaErrors(cudaFree(devReduceMin));
	customCheckCudaErrors(cudaFree(devFinalMin));
	customCheckCudaErrors(cudaFree(devFinalMax));
...
/* 	customCheckCudaErrors(cudaMemcpy(devMatrix, hostMatrix, nPixels * nDimension * sizeof(float), cudaMemcpyHostToDevice)); */
...
//  customCheckCudaErrors(cudaMemcpy(p_eigenvectors, *devEigVectors+10*nMatrixDimension, width*height*sizeof(float), cudaMemcpyDeviceToHost));
