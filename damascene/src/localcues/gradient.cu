...
float* getImage(uint width, uint height, float* devImage) {
  ...
  customCheckCudaErrors(cudaMemcpy(result, devImage, imageSize, cudaMemcpyDeviceToHost));
  return result;
}
...
int initializeGradients(uint widthIn, uint heightIn, uint borderIn, uint maxbins, uint norientsIn, uint nscaleIn, uint textonChoice) {
  ...
  customCheckCudaErrors(cudaMalloc((void**)&devGradients, sizeof(float) * norients * nscale * borderWidth * borderHeight));
  customCheckCudaErrors(cudaMalloc((void**)&devQuantized, sizeof(int) * width * height));
  customCheckCudaErrors(cudaMalloc((void**)&devMirrored, sizeof(int) * borderWidth * borderHeight));
  customCheckCudaErrors(cudaMalloc((void**)&devGradientA, sizeof(float) * width * height));
  customCheckCudaErrors(cudaMalloc((void**)&devGradientB, sizeof(float) * width * height));
  ...
  customCheckCudaErrors(cudaMalloc((void**)&devTurned, sizeof(int) * maxWidth * maxHeight));
  customCheckCudaErrors(cudaMalloc((void**)&devIntegrals, sizeof(int) * binPitch * maxWidth * maxHeight));
  return binPitch;
}


void finalizeGradients() {
  customCheckCudaErrors(cudaFree(devIntegrals));
  customCheckCudaErrors(cudaFree(devTurned));
  customCheckCudaErrors(cudaFree(devGradientB));
  customCheckCudaErrors(cudaFree(devGradientA));
  customCheckCudaErrors(cudaFree(devMirrored));
  customCheckCudaErrors(cudaFree(devQuantized));
  customCheckCudaErrors(cudaFree(devGradients));
}
