...
{
  //    customCheckCudaErrors(cudaUnbindTexture(tex_parabola_pixels));
    customCheckCudaErrors(cudaFreeArray(cuda_parabola_pixels));
    customCheckCudaErrors(cudaFree(cuda_parabola_trace));
    //customCheckCudaErrors(cudaFree(cuda_parabola_filters));
}
...
static inline void copy_cuda_parabola_buffers(int norients, int width, int height, int border, float *devPixels, int filter_radius, int filter_length, float* host_filters)
{
    ...
    //customCheckCudaErrors(cudaMalloc((void**)&cuda_parabola_filters, sizeof(float)*filter_length*filter_length*norients));
    //customCheckCudaErrors(cudaMemcpy(cuda_parabola_filters, host_filters, filter_length*filter_length*norients*sizeof(float), cudaMemcpyHostToDevice));
...
static inline void cuda_parabola_kernel(int norients, int width, int height, int border, int filter_radius, int filter_length, float* devResult, int cuePitchInFloats)
{
    ...
    customCheckCudaErrors(cudaBindTextureToArray(tex_parabola_pixels, cuda_parabola_pixels) );
    ...
    customCheckCudaErrors(cudaUnbindTexture(tex_parabola_pixels));
    //customCheckCudaErrors(cudaUnbindTexture(tex_parabola_filters));
