...
// yunsup fixed
  ...
  log_info("afLocal=%d, afCopies=%d", afLocal, afCopies);
  ...
  int* devCentroidMass;
  customCheckCudaErrors(cudaMalloc((void**)&devCentroidMass, sizeof(int) * filterCount * clusterCount* afCopies));
  unsigned int* devCentroidCount;
  customCheckCudaErrors(cudaMalloc((void**)&devCentroidCount, sizeof(unsigned int) * filterCount * clusterCount* afCopies));
  float* devCentroids;
  customCheckCudaErrors(cudaMalloc((void**)&devCentroids, sizeof(float) * filterCount * clusterCount));
  int* devChanges;
  customCheckCudaErrors(cudaMalloc((void**)&devChanges, sizeof(int)));

  float* devPointsDots;
  customCheckCudaErrors(cudaMalloc((void**)&devPointsDots, sizeof(int) * nPixels));
  float* devCentroidsDots;
  customCheckCudaErrors(cudaMalloc((void**)&devCentroidsDots, sizeof(int) * clusterCount));
  makeSelfDots<<<linearGrid, linearBlock>>>(devResponses, nPixels, devPointsDots, nPixels, filterCount);

  float* devDist;
  size_t devDistPitch;
  customCheckCudaErrors(cudaMallocPitch((void**)&devDist, &devDistPitch, sizeof(float) * nPixels, clusterCount));
  int devDistPitchInFloats = devDistPitch/sizeof(float);

  int i;
  int hostChanges;
  for(i = 0; i < maxIter; i++) {
    customCheckCudaErrors(cudaMemset(devCentroidMass, 0, sizeof(int) * filterCount * clusterCount* afCopies));
    customCheckCudaErrors(cudaMemset(devCentroidCount, 0, sizeof(int) * clusterCount* afCopies));
    customCheckCudaErrors(cudaMemset(devChanges, 0, sizeof(int)));
    ...
    customCheckCudaErrors(cudaMemcpy(&hostChanges, devChanges, sizeof(int), cudaMemcpyDeviceToHost));
    //printf("\tChanges: %d\n", hostChanges);
    log_info(">+< \tChanges: %d", hostChanges);
    if (hostChanges <= convThresh) {
      break;
    }
  }
  //printf("\t%i iterations until termination\n", i);
  log_info(">+< \t%i iterations until termination", i);
  cudaFree(devDist);
