...
//  delete[] f;
  ...
  customCheckCudaErrors(cudaMalloc((void**)&devResponses, sizeof(float)*nPixels*filterCount));
  //customCheckCudaErrors(cudaMemcpyToSymbol(radii, hRadii, sizeof(hRadii)));
  customCheckCudaErrors(cudaMemcpyToSymbol(radii, hRadii, filterCount*sizeof(int)));
  //customCheckCudaErrors(cudaMemcpyToSymbol(coefficients, hCoefficients, sizeof(hCoefficients)));
  //customCheckCudaErrors(cudaMemcpyToSymbol(coefficients, hCoefficients, nFilterCoefficients* sizeof(float)));
  
  float* devcoefficients;
  customCheckCudaErrors(cudaMalloc((void**)&devcoefficients, nFilterCoefficients* sizeof(float)));
  customCheckCudaErrors(cudaMemcpy(devcoefficients, hCoefficients, nFilterCoefficients* sizeof(float), cudaMemcpyHostToDevice));
  ...
  customCheckCudaErrors(cudaMallocArray(&imageArray, &floatTex, width, height));
  customCheckCudaErrors(cudaMemcpyToArray(imageArray, 0, 0, devImage, nPixels * sizeof(float), cudaMemcpyDeviceToDevice));
  customCheckCudaErrors(cudaBindTextureToArray(image, imageArray));
  //printf("Convolving\n");
  log_info("Convolving");
  ...
  customCheckCudaErrors(cudaFreeArray(imageArray));
  customCheckCudaErrors(cudaFree(devResponses));
