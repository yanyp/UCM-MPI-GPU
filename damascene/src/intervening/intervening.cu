...
/*   mPb.normalized = 0; */
  ...
  cudaArray* mPbArray;
  customCheckCudaErrors(cudaMallocArray(&mPbArray, &channelDesc, width, height));
  customCheckCudaErrors(cudaBindTextureToArray(mPb, mPbArray));
  customCheckCudaErrors(cudaMemcpy2DToArray(mPbArray, 0, 0, devMPb, sizeof(float) * width, sizeof(float) * width, height, cudaMemcpyDeviceToDevice));
  size_t devMatrixPitch;
  customCheckCudaErrors(cudaMallocPitch((void**)p_devMatrix, &devMatrixPitch, nPixels * sizeof(float), nDimension));
  assert(devMatrixPitch == theStencil.getMatrixPitch());
  float* devMatrix = *p_devMatrix;
  customCheckCudaErrors(cudaMemset(devMatrix, 0, devMatrixPitch * nDimension));
  int hostDiagonalMap[CONSTSPACE];
  theStencil.copyDiagonalOffsets(hostDiagonalMap);
  customCheckCudaErrors(cudaMemcpyToSymbol(constDiagonals, hostDiagonalMap, sizeof(int) * diameter * diameter));
  ...
  customCheckCudaErrors(cudaUnbindTexture(mPb));
  customCheckCudaErrors(cudaFreeArray(mPbArray));
}
