...
{
	customCheckCudaErrors(cudaMalloc((void**)p_devMaxPB, p_nSize * sizeof(float)));
	customCheckCudaErrors(cudaMalloc((void**)p_devOri, p_nSize * sizeof(int)));
}

void freeCudaArrays(float* p_devMaxPB, int* p_devOri)
{
	customCheckCudaErrors(cudaFree(p_devMaxPB));
	customCheckCudaErrors(cudaFree(p_devOri));
}
	...
	for (unsigned long x = 0; x < size_x; x++) {
		for (unsigned long y = 0; y < size_y; y++) {
			...
			if (v < 0) {
				...
				log_warn("matrix must be nonnegative");
      }
			/* initialize indices of values in local neighborhood */
...
void nonMaxSuppression(int p_nWidth, int p_nHeight, float* p_devPB, int p_nDevPBPitch, float* devNMax)
{
	...
	//customCheckCudaErrors(cudaMemcpy(maxpb, devMaxPB, size * sizeof(float), cudaMemcpyDeviceToHost));
	//customCheckCudaErrors(cudaMemcpy(ori, devOri, size * sizeof(int), cudaMemcpyDeviceToHost));
	...
	log_info("Max time: %f seconds", solveTime);
	...
	customCheckCudaErrors(cudaUnbindTexture(texMax));
	customCheckCudaErrors(cudaUnbindTexture(texOrient));
	...
	log_info("Oriented Max time: %f seconds", solveTime);
	...
	log_info("Solve time: %f seconds", solveTime);
